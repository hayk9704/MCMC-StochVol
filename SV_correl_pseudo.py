...
from MCMC_functions import theta_to_x, stochvol, log_prior
...
def correl_pseudo_MCMC(ys, rho = 0.8, N_mcmc = 20000, x_first = theta_to_x(), x_std = 0.01, m_latent = 500, burnin = 2000):
...
if __name__ == "__main__":
    real_pars = {"mu": -0.86, "sigma2_eta": 0.025, "phi": 0.86}

    stochvol.generate(mu = real_pars["mu"], phi = real_pars["phi"], sigma2_eta = real_pars["sigma2_eta"], T = 140, seed = 101)
    y_gen = stochvol.ys
    h_gen = stochvol.hs
    values = correl_pseudo_MCMC(ys = y_gen, N_mcmc = 30000, m_latent = 10, burnin = 10000, x_std = 0.4, rho = 0.99) 
    ...
    print(f"mu_mean: {mu_mean} \nsample mean (mean(h_gen)): {np.mean(h_gen)} \n \nsigma2_mean: {sigma2_mean} \nphi_mean: {phi_mean} \
           \n \nlong term std estimate: {np.sqrt(sigma2_mean/(1-phi_mean**2))} \nsample std: {np.std(h_gen)} \n \nacc ratio: {acc_ratio}")
