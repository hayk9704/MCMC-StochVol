...
# the code to verify the number of particles for PMMH and correlated PMMH


def correl_m_check(ys, N_mcmc = 20000, x = xstart(), m_latent = 50, burnin = 2000, rho = 0.9):
...
if __name__ =="__main__":
    ...
    m_particles = 200                                        # number of particles to check
    T_obs = 700                                                 # number of obs.
    real_pars = {"mu": -0.86, "sigma2_eta": 0.0225, "phi": 0.98}
    stochvol.generate(mu = real_pars["mu"], phi = real_pars["phi"], sigma2_eta = real_pars["sigma2_eta"], T = T_obs, seed = None)
    y_gen = stochvol.ys
    par_vector = xstart(mu = -0.86, phi = 0.98, sigma2_eta = 0.0225)  
    correl_m_check(ys = y_gen, N_mcmc = 500, x = par_vector, m_latent = m_particles, burnin = 50, rho = 0.995)
