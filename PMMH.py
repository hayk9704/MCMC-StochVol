...
"""
This code applies the correlated PMMH algorithm to the SV model - diagonal gaussian proposal
set rho = 0 to get the standard (non-correlated) PMMH
"""
...
def PMCMC(ys, N_mcmc = 20000, x_first = xstart(), s = 2.38**2/3, m_latent = 50, burnin = 2000, rho = 0.9):
    ...
    for i in range(N_mcmc-1):
        ...
        acc = newlogpost - oldlogpost                                   # recording acceptance ratio
            
        
        u = np.random.uniform()                                         # uniform draw to approve acceptance
        if np.log(u) < acc:
            xold = xnew
            oldloglik = newloglik
            U_old = U_new
