# Here I am trying to make the correlated PMMH more efficient.

'''
np.argsort is a NumPy function that returns the indices that would sort an array. 
Instead of returning the sorted array itself, it gives you the order of indices that would arrange the array in ascending order.
'''
...
def particles0(y_t, theta, m_latent, randomness):
    ...
    h = h[np.argsort(h)]                                            # reordering h in ascending order to apply Hilbert sort
